...
const BrandModal = ({ id }) => {
    ...
    const handleModal = async (id) => {
        ...
        const res = await axiousSecure.get(`/brand/${id}`)
    ...
    const onSubmit = (data) => {
        ...
        const res = axiousSecure.patch(`/updateBrand/${id}`, brandData)
        ...
        <div>
            ...
            <div className="mx-auto w-fit">
                <div onClick={() => setOpenModal(false)} className={`fixed z-[100] w-screen ${openModal ? 'visible opacity-100' : 'invisible opacity-0'} inset-0 grid place-items-center bg-black/20 backdrop-blur-sm duration-100 dark:bg-transparent`}>
                    <div onClick={(e_) => e_.stopPropagation()} className={`absolute w-10/12 md:w-96 rounded-lg bg-white p-6 drop-shadow-lg dark:bg-zinc-900 dark:text-white ${openModal ? 'opacity-1 duration-300' : 'scale-110 opacity-0 duration-150'}`}>
                        ...
                        <h1 className="mb-2 text-sm md:text-2xl font-semibold text-orange-300 text-center">UPDATE BRAND!</h1>
